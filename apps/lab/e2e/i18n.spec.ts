...
test.describe('Lab localization (issue #43)', () => {
	test('explicit query overrides storage and locale switching preserves unrelated query/hash', async ({ context, page }) => {
		...
		await expect(localeSelect(page)).toHaveValue('zh-TW')
		await expect(page.locator('html')).toHaveAttribute('lang', 'zh-TW')
		await expect(page.getByRole('button', { name: '套用', exact: true })).toBeVisible()
		expect(await page.evaluate(key => localStorage.getItem(key), LOCALE_STORAGE_KEY)).toBe('zh-TW')
		await expect(page).toHaveURL(/\?lang=zh-TW&probe=keep#anchor$/)
		...
		await expect(page).toHaveURL(/\?lang=en&probe=keep#anchor$/)
	...
	test('stored zh-TW is used when query is absent and canonicalized into the URL', async ({ context, page }) => {
		...
		await expect(localeSelect(page)).toHaveValue('zh-TW')
		await expect(page).toHaveURL(/\?probe=stored&lang=zh-TW#state$/)
		await expect(page.locator('html')).toHaveAttribute('lang', 'zh-TW')
	})
	...
	test('changing locale preserves live Runtime state and shared inspector focus', async ({ page }) => {
		...
		await expect(page.getByTestId('blueprint-view-implementation')).toBeVisible()

		await localeSelect(page).selectOption('zh-TW')
	...
	test('changing locale translates the current tutorial step without restarting tutorial progress', async ({ page }) => {
		...
		await expect(rail.getByText('This is the Interactive Survey', { exact: true })).toBeVisible()
		await expect(rail.getByText('Step 1 of 9', { exact: true })).toBeVisible()

		await localeSelect(page).selectOption('zh-TW')

		const translatedRail = page.getByRole('complementary', { name: '教學' })
		await expect(translatedRail.getByText('這是互動式問卷', { exact: true })).toBeVisible()
		await expect(translatedRail.getByText('第 1 步，共 9 步', { exact: true })).toBeVisible()
	})
})
