...
test('valid flow: Submit then Generate result renders the Recommendation block', async ({ page }) => {
	...
	await preview.getByRole('button', { name: 'Generate result' })
		.click()
	const recommendation = preview.locator('[data-tutorial-target="survey-recommendation"]')
	await expect(recommendation)
		.toBeVisible()
	await expect(recommendation.getByRole('heading', { name: 'Recommendation' }))
		.toBeVisible()
	await expect(recommendation.locator('p')
		.filter({ hasText: 'Destination:' }))
		.toHaveText('Destination: tokyo · requested style: balanced · recommended style: balanced · fit: comfortable')
	await expect(recommendation.locator('p')
		.filter({ hasText: 'Trip days:' }))
		.toHaveText('Trip days: 5 · travelers: 2')
	await expect(recommendation.locator('p')
		.filter({ hasText: 'Budget:' }))
		.toHaveText('Budget: 1800 vs. estimated baseline cost: 1500.00 (gap 300.00) · budget/person/day: 180.00')
})
