...
test.describe('Lab theme (issue #44)', () => {
	test('switches document, Monaco, and Dockview presentation without resetting Runtime state', async ({ context, page }) => {
		...
		await expect(page.locator('html')).toHaveAttribute('data-lab-theme', 'light')
		await expect(page.locator('.monaco-editor')).toBeVisible()

		const monacoLight = await page.locator('.monaco-editor').evaluate(element => getComputedStyle(element).backgroundColor)
		const dockviewLight = await page.locator('.dockview-theme-lab').evaluate(element => getComputedStyle(element).getPropertyValue('--dv-group-view-background-color'))

		await page.getByRole('button', { name: '+1', exact: true }).click()
		await expect(page.getByText('count: 1 · doubled: 2', { exact: true })).toBeVisible()

		await themeSelect.selectOption('dark')
		await expect(page.locator('html')).toHaveAttribute('data-lab-theme', 'dark')
		await expect(page.getByText('count: 1 · doubled: 2', { exact: true })).toBeVisible()
		expect(await page.evaluate(key => localStorage.getItem(key), THEME_STORAGE_KEY)).toBe('dark')

		await expect.poll(async () => page.locator('.monaco-editor').evaluate(element => getComputedStyle(element).backgroundColor))
			.not.toBe(monacoLight)
		const dockviewDark = await page.locator('.dockview-theme-lab').evaluate(element => getComputedStyle(element).getPropertyValue('--dv-group-view-background-color'))
		expect(dockviewDark).not.toBe(dockviewLight)
	...
	test('rehighlights already-loaded Implementation source when theme changes without reloading source text', async ({ context, page, blockedRequestUrls }) => {
		...
		await page.getByRole('button', { name: 'title : Text' }).click()
		await page.getByTestId('blueprint-view-implementation').click()

		const code = page.getByTestId('implementation-code')
		const pre = code.locator('pre')
		await expect(pre).toBeVisible()
		await expect(code).toContainText("createWidgetPlugin('Text')")
		...
		expect(await code.textContent()).toBe(textBefore)
		expect(blockedRequestUrls).toEqual([])
	})
})
...
test.describe('native dialog theme foreground (issue #44)', () => {
	...
	for (const selectedTheme of ['light', 'dark'] as const) {
		test(`welcome dialog inherits readable foreground in ${selectedTheme} theme`, async ({ context, page }) => {
			...
			expect(colors.heading).toBe(colors.body)
		})
	}
})
