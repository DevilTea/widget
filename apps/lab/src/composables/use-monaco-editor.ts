 ...
 * - theme and JSON grammar are bundled from `tm-themes`/`tm-grammars` (no CDN dependency);
 ...
 *   anything Monaco/LSP reports here.
 *
...
function ensureMonaco(): Promise<Monaco> {
	monacoPromise ??= import('modern-monaco').then(({ init }) => init({
